"""Consumer-only loader for the physical v13 fit-token view."""
...
def _event_rows(value: object) -> tuple[FormalTokenSubsetEventBinding, ...]:
    ...
    for row in value:
        ...
        event_id, patient_id, record_sha, pre_sha, manifest_sha, tensor_sha, relative = row
        ...
        rows.append(
            FormalTokenSubsetEventBinding(
                ...
                event_record_sha256=require_sha256(
                    record_sha, field="event_record_sha256"
                ),
                preprocess_receipt_sha256=require_sha256(
                    pre_sha, field="preprocess_receipt_sha256"
                ),
                bundle_path=Path("/") / str(relative),
                bundle_manifest_sha256=require_sha256(
                    manifest_sha, field="bundle_manifest_sha256"
                ),
                tensor_sha256=require_sha256(tensor_sha, field="tensor_sha256"),
            )
        )
    result = tuple(rows)
    if tuple(sorted(result, key=lambda item: (item.patient_id, item.event_id))) != result:
...
def load_fit_token_view_v13(
    ...
    *,
    ...
    manifest, manifest_sha = _strict_json(
        source / FIT_TOKEN_VIEW_MANIFEST,
        expected_sha256=expected_manifest_sha256,
    )
    receipt, receipt_sha = _strict_json(
        source / FIT_TOKEN_VIEW_RECEIPT,
    ...
    _, fold = selection(manifest["selection"])
    if manifest["oof_fold"] != fold:
        raise ValueError("fit-token view selection/fold mismatch")
    fit = patient_roster(
        manifest["fit_patient_ids"], field="fit_patient_ids", allow_empty=False
    )
    gate = patient_roster(
    ...
    for field in (
        ...
        require_sha256(manifest[field], field=field)
    templates = _event_rows(manifest["fit_event_rows"])
    if len(templates) != manifest["fit_event_count"]:
        raise ValueError("fit-token view event count changed")
    if {row.patient_id for row in templates} != set(fit):
    ...
    if {item.name for item in events_dir.iterdir()} != {
        row.event_id for row in templates
    }:
    ...
    for template in templates:
        ...
        if (
            ...
            or {item.name for item in bundle.iterdir()} != _BUNDLE_FILES
        ):
        ...
        checks = (
            ...
            == manifest["foundation_modeling_sha256"],
    ...
    corpus = _issue_verified_formal_token_corpus_subset_artifact(
        ...
        preprocessing_selection_artifact_sha256=str(
            manifest["preprocessing_selection_artifact_sha256"]
        ),
        preprocessing_selection_bundle_receipt_sha256=str(
            manifest["preprocessing_selection_bundle_receipt_sha256"]
        ),
        preprocessing_protocol_receipt_sha256=str(
            manifest["preprocessing_protocol_receipt_sha256"]
        ),
        preprocessing_selected_arm_result_receipt_sha256=str(
            manifest["preprocessing_selected_arm_result_receipt_sha256"]
        ),
...
__all__ = ("LoadedFitTokenViewV13", "load_fit_token_view_v13")
