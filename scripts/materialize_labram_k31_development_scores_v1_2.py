#!/usr/bin/env python3
"""Materialize target-free scores from LaBraM k31 v1.2 producers only.

The CLI has no argument for a DeepSOZ source/target table, private data,
source-eval corpus, caller score, label, mask, event row, or fold assignment.
Target-v2 hashes bind the already verified signal receipt; target values are
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=_run_spec,
        required=True,
        help="repeat exactly once for v1.2 fold0..fold4 and final",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    artifact = materialize_labram_k31_development_scores_v1_2(
        recovery_runs=runs,
        protocol=protocol,
        timeline=timeline,
        ...
        json.dumps(
            {
                ...
                "producer_bundle_schema_version": artifact.manifest[
