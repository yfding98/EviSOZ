#!/usr/bin/env python3
"""Train the fixed primary v13 capacity-matched channel-only control.

The grouped-k1 residual has the same 4,352 extra parameters as the k31
temporal block but operates independently at every edge-second.  This runner
has no control-kind switch: it can instantiate only that primary comparator.
"""
...
from scripts.train_labram_ictal_matched_independent_control_v13 import (  # noqa: E402
    _execution_receipt,
    _memoize_fit_only,
)
    ...
    *,
    ...
    with ictal_determinism_runtime(config, execution_device_type=device.type):
        with torch.random.fork_rng(devices=cuda_devices):
            ...
            head = CapacityMatchedChannelResidualIctalInvolvementHead().to(device)
            ...
            for epoch in range(config.fixed_epochs):
                ...
                print(
                    json.dumps(
                        {
                            "stage": "v13_fit_only_capacity_matched_channel_training",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    dataset = build_fit_only_token_bag_dataset_v13(
        fit_token_view.corpus, fit_targets
    )
    if dataset.patient_ids != fit_patients:
        raise ValueError("Fit-only dataset changed the exact k31 fit roster")
    preflight = {
        "schema_version": "soz_labram_ictal_capacity_matched_control_preflight_v13",
        "candidate": V13_CAPACITY_MATCHED_CONTROL_CANDIDATE,
        "selection": lineage.selection,
        "oof_fold": lineage.oof_fold,
        "preflight_passed": True,
        "training_started": False,
        "primary_capacity_matched_confirmation_control": True,
        "head_temporal_mixing": False,
        "head_extra_parameter_count": 4352,
        "matched_k31_extra_parameter_count": 4352,
        ...
        "source_broker_module_imported": False,
    ...
    head, training_run = _train_fit_only(
        _memoize_fit_only(dataset),
        config=config,
        device=device,
        selection=args.selection,
    )
    saved = save_capacity_matched_channel_control_v13(
        output,
        lineage=lineage,
        head=head,
