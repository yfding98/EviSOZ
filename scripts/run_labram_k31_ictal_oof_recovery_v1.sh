...
# Run one post-formal-v5 LaBraM k31 OOF recovery selection.  The candidate,
# optimizer, seed, epochs, I-gate exclusion, and all data identities are fixed
# in the Python entry point and the pinned arguments below.
...
if [[ $# -eq 2 ]]; then
  if [[ "$2" != "--preflight-only" ]]; then
    echo "second argument must be --preflight-only" >&2
    exit 2
  fi
...
case "$SELECTION" in
  fold0)
    ...
    TRAINING_MANIFEST_BUNDLE="outputs/tusz_ictal_oof_fold_manifests_v4_1_20260809_current_preflight/fold_0"
  ...
  fold1)
    ...
    TRAINING_MANIFEST_BUNDLE="outputs/tusz_ictal_oof_fold_manifests_v4_1_20260809_current_preflight/fold_1"
  ...
  fold2)
    ...
    TRAINING_MANIFEST_BUNDLE="outputs/tusz_ictal_oof_fold_manifests_v4_1_20260809_current_preflight/fold_2"
  ...
  fold3)
    ...
    TRAINING_MANIFEST_BUNDLE="outputs/tusz_ictal_oof_fold_manifests_v4_1_20260809_current_preflight/fold_3"
  ...
  fold4)
    ...
    TRAINING_MANIFEST_BUNDLE="outputs/tusz_ictal_oof_fold_manifests_v4_1_20260809_current_preflight/fold_4"
  ...
  final)
    ...
    ;;
  *)
    echo "selection must be fold0..fold4 or final" >&2
    exit 2
    ;;
esac
...
COMMON_ARGS=(
  ...
  --output-directory "outputs/labram_ictal_k31_oof_recovery_v1_1_20260810/$SELECTION"
  --device cuda
)

rtk mkdir -p outputs/labram_ictal_k31_oof_recovery_v1_1_20260810

if [[ "$SELECTION" == "final" ]]; then
  rtk python3 scripts/train_labram_k31_ictal_oof_recovery_v1.py \
    "${COMMON_ARGS[@]}" \
