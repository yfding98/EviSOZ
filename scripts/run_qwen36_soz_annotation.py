#!/usr/bin/env python3
"""Run the label-neutral preselector plus local Qwen3.6 SOZ expert review.

The wrapper pins the verified offline multimodal settings while forwarding all
dataset/event selection arguments to ``llm_soz_annotator``.  Source manifest
times are navigation hints only; source spatial labels and AutoLabel outputs
are not supplied as conclusions to Qwen.
"""
...
DEFAULT_MODEL = ROOT / "models" / "Qwen3.6-35B-A3B-GPTQ-Int4"
...
def _validate_complete_model(model_path: Path) -> None:
    ...
    # Hugging Face may retain stale resume markers under its private .cache
    # even after the indexed final shard was independently downloaded and
    # checksum-verified.  Indexed final files are authoritative; only an
    # incomplete marker outside .cache can invalidate the runnable directory.
    incomplete = [
        path
    ...
    try:
        ...
        pass
    correct_base_model = "Qwen/Qwen3.6-35B-A3B" in readme
    if (
        missing
        or index_error
        ...
        raise SystemExit(
            "Qwen3.6 model validation failed: "
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    configured_model = os.environ.get("QWEN36_MODEL_PATH", "").strip()
    model_path = Path(configured_model).expanduser() if configured_model else DEFAULT_MODEL
    model_path = model_path.resolve()
    _validate_complete_model(model_path)
    if "--output-dir" not in user_args:
        user_args.extend(
            ["--output-dir", str(ROOT / "outputs" / f"qwen36_soz_{dataset}")]
        )
    defaults = [
        "--inference-backend",
        "local_transformers",
