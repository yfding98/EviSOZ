...
# Train the frozen CE6 morphology head from the one authorization-bound master
# corpus.  The three corpus identities are deliberately supplied only after
# atomic corpus publication; all cohort, split, preprocessing, foundation, and
# optimization choices remain fixed by the audited Python entry point.

if [[ $# -ne 3 ]]; then
  echo "usage: $0 BUNDLE_MANIFEST_SHA256 PRODUCER_RECEIPT_SHA256 TOKEN_INDEX_SHA256" >&2
  exit 2
fi

BUNDLE_MANIFEST_SHA256="$1"
PRODUCER_RECEIPT_SHA256="$2"
TOKEN_INDEX_SHA256="$3"
for value in \
  "$BUNDLE_MANIFEST_SHA256" \
  "$PRODUCER_RECEIPT_SHA256" \
  "$TOKEN_INDEX_SHA256"; do
  if [[ ! "$value" =~ ^[0-9a-f]{64}$ ]]; then
    echo "all corpus identities must be lowercase SHA-256 digests" >&2
    exit 2
  fi
done

rtk python3 scripts/train_tuev_morphology_concept.py \
  ...
  --holding-manifest-bundle outputs/tuev_morphology_holding_manifest_v3_20260810 \
  --expected-holding-bundle-manifest-sha256 0f6d43a9e2747e2bfd5bce207c0209796f619fec4ad27ee8b04b9d0a44714623 \
  --expected-holding-source-manifest-sha256 366eb0e99a751113814ea1fd424bbb0844dc205d8a2db647d9db9e26a369e440 \
  ...
  --labram-modeling-path /mnt/hd1/dyf/workspace/LaBraM/modeling_finetune.py \
  --labram-checkpoint-path /mnt/hd1/dyf/workspace/LaBraM/checkpoints/labram-base.pth \
  --device cuda \
  --master-corpus outputs/tuev_morphology_master_corpus_formal_v1_20260809 \
  --expected-master-corpus-bundle-manifest-sha256 "$BUNDLE_MANIFEST_SHA256" \
  --expected-master-corpus-producer-receipt-sha256 "$PRODUCER_RECEIPT_SHA256" \
  --expected-master-corpus-token-index-sha256 "$TOKEN_INDEX_SHA256" \
  --output-directory outputs/tuev_morphology_concept_formal_v1_20260810
